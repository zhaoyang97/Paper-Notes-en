...
- **Interpretability + Dynamic Efficiency**: Sparse lexical representations allow error tracing (e.g., discovering "Momo" was missed by the English view but recovered with a high weight for "Mo" by LexEcho). Using mass-based pruning to compress document representations to an average of only 30 active dimensions, MILCO-560M still outperforms the 1024-dimensional Qwen3-0.6B, with 3× lower retrieval latency and 10× smaller index.
