...
- **State of the Field**: Screen-camera (SC) capture is one of the primary means of unauthorized content acquisition, and robust watermarking is a core tool for post-hoc copyright tracing. The key to existing SC watermarking methods lies in the noise layer design during training—the noise layer simulates SC degradations so that the watermarking network learns to resist them through adversarial training.
