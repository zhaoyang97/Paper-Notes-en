...
**State of the Field**: Neural combinatorial optimization solvers (e.g., POMO, INViT) learn VRP solution policies directly via deep reinforcement learning and have achieved performance competitive with classical heuristics (e.g., LKH3) on fixed-distribution, fixed-scale tasks. The dominant training paradigm is one-shot training on predefined tasks.
