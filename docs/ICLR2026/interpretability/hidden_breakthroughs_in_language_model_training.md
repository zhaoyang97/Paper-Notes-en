...
**Root Cause**: A smooth aggregate loss curve ≠ absence of breakthroughs. Saxe et al. (2019) theoretically predicted that multiple sigmoidal phase transitions occurring at different times superpose into a smooth curve. The missing piece is a tool to recover these hidden phase transitions from the smooth curve.

**Paper Goals**: (1) How can conceptual breakthroughs during training be discovered automatically without predefining skills? (2) How can the entanglement of a single sample undergoing multiple skill breakthroughs simultaneously be handled? (3) How can loss changes be decomposed onto interpretable gradient directions?

**Starting Point**: The authors observe that the training subspace is low-rank and that linearly interpolated checkpoints preserve semantic capabilities, suggesting that linear decomposition is meaningful at the conceptual level. Projecting loss changes onto the high-curvature directions of the Hessian may allow each direction to correspond to the acquisition of a distinct "skill," thereby decoupling a single sample's loss change into contributions along independent directions.
