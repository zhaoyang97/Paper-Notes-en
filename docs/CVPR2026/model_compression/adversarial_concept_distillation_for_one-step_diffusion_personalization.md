...
1. **State of the Field**: Large-scale generative models dominate T2I generation, and personalized generation (i.e., learning new concepts) is an important application. Distillation-based acceleration techniques can already compress inference to a single step.
...
4. **Paper Goals**: Design the first framework capable of reliable, high-quality personalization on one-step diffusion models.
5. **Starting Point**: Treat personalization and acceleration as a joint optimization problem rather than a two-stage sequential pipeline.
