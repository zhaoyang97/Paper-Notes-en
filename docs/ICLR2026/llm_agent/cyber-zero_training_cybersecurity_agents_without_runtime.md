...
On the evaluation side, the authors made an engineering contribution with **ENIGMA+** (an enhancement of the ENIGMA scaffold): ① All evaluation tasks are **executed in parallel** (each Docker container has an independent network interface and isolated environment), reducing the 300+ challenge evaluation time from ENIGMA's 1–3 days to under 5 hours; ② A **unified maximum interaction limit (40 turns)** replaces ENIGMA's cost-based ($3/challenge) budget, allowing fair comparison between models with different pricing; ③ A Simple Summarizer replaces the LLM Summarizer because binary decompilation output can be excessively long, exceeding context limits. Additionally, the authors manually fixed approximately 6% of existing CTF benchmark challenges containing errors.
