...
3. **Root Cause**: The multimodal nature of information in long videos (text cannot fully express visual details) and its multi-scale temporal structure (different events span different time ranges) demand that memory and retrieval systems be adaptive in both modality and scale, whereas existing methods are fixed in both dimensions.
4. **Paper Goals**: (1) How can both textual and visual memory be leveraged simultaneously to support reasoning? (2) How can information be retrieved across multiple temporal scales? (3) How can the model autonomously decide when to consult text, when to consult images, and when to stop?
5. **Starting Point**: By analogy with the human memory system—episodic memory stores specific events, semantic memory stores abstract knowledge, and visual memory retains sensory details—three complementary memory types are constructed and dynamically combined through an iterative retrieval agent.
