...
- **Using SAM as the discriminator backbone**: Utilizing the frozen visual encoder of a general-purpose Segmentation Foundation Model (SAM) as a pixel-level discriminator leverages its sensitivity to local geometry/texture to suppress mode-seeking. This is a highly transferable trick applicable to other GAN/adversarial distillation setups.
