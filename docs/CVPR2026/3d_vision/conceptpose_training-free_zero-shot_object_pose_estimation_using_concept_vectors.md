...
The problem is defined as **relative pose estimation from a single reference viewpoint**: given two pose-less RGB-D observations of the same object—an anchor frame $A=\{I_a,D_a,M_a,K_a\}$ and a query frame $Q=\{I_q,D_q,M_q,K_q\}$ (where $I$ is RGB, $D$ is depth, $M$ is object mask, and $K$ is intrinsic parameters), plus a category name $c$ (e.g., "cup")—the goal is to estimate the 6DoF camera-to-camera transformation $T_{rel}=(R_{rel},t_{rel})$ such that corresponding 3D points satisfy $P_q = R_{rel}\cdot P_a + t_{rel}$. At inference, **no dataset training, CAD models, or pose ground truths are used**.
