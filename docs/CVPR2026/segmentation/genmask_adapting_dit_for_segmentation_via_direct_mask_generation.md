...
1. **State of the Field**: Text-guided segmentation is a core task in computer vision. Leveraging pretrained generative models (e.g., diffusion models) for segmentation has become a popular direction. Existing methods typically use pretrained diffusion models as backbone networks, extracting hidden features from the denoising or diffusion inversion process and feeding them into trainable task-specific decoders to obtain segmentation masks.
