...
**Goal**: Without performing 3D reconstruction, how can a diffusion model directly learn 3D-aware object manipulation? How can scene consistency be maintained across multiple editing rounds? How can physically plausible background effects be generated?

**Key Insight**: The editing process is modeled as a learned 3D transformation sequence, leveraging the motion priors of a pretrained video diffusion model (SVD). A hybrid dataset (real domain + synthetic domain) is constructed to provide multi-round editing training sequences, with Domain LoRA used to isolate domain-specific content.
