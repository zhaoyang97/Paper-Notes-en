...
**Root Cause**: Although prefix parsing is theoretically closely related to ordinary parsing, in practice it requires entirely different implementations, creating a disconnect between theoretical elegance and engineering complexity.

**Paper Goals**: To provide a simple, general, and efficient framework that fully reduces prefix parsing to ordinary parsing.

**Starting Point**: Grammar transformation — given grammar $G$, construct a new grammar $G'$ such that $L(G')$ equals exactly the set of all prefix strings of $L(G)$.
