...
3. **Root Cause**: Robots must detect subtle scene changes (e.g., a robotic arm moving a few centimeters), which demands that visual states simultaneously encode "what objects are present" and "where they are." Existing SSL methods either discard spatial information (augmentation invariance in contrastive learning) or do not produce a single compact representation (patch-level methods output token sequences).
4. **Paper Goals**: Learn a single-token visual state representation that encodes complete "what-is-where" scene composition—which semantic entities exist, where they are, and how they are arranged.
5. **Starting Point**: If a compact representation can recover the content of an arbitrary local crop from minimal cues, it must encode both the semantic and spatial information of the entire scene.
