...
**State of the Field**: Differentially private continual release is a fundamental model for privacy-preserving data processing, where data arrives as a stream and the algorithm must privately publish the underlying statistics at each time step. The turnstile model allows both insertions and deletions, making it significantly harder than the insertion-only setting. Two foundational streaming problems—distinct elements counting and $F_2$ moment estimation—are classical cornerstones of the streaming algorithms literature.
