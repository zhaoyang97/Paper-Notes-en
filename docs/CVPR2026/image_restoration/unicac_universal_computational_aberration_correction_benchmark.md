...
1. **State of the Field**: Computational Aberration Correction (CAC) is a classical problem in computational imaging. Existing methods are typically designed for specific optical systems and require retraining when applied to new lenses.
...
3. **Root Cause**: Universal CAC demands zero-shot generalization to unseen lenses, yet commercial lens design parameters are rarely publicly available, making it difficult to construct large-scale, diverse training and test data.
4. **Paper Goals**: (1) Construct a large-scale CAC benchmark covering both spherical and aspherical lenses; (2) propose a more reliable aberration quantification framework; (3) systematically evaluate existing methods and summarize key findings.
5. **Starting Point**: Automatic Optical Design (AOD) methods are leveraged to generate a large number of physically constrained lens prescriptions, circumventing the inaccessibility of commercial lens designs.
