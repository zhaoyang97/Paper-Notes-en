...
3. **Root Cause**: Joint learning of geometry and appearance makes models highly sensitive to geometry prediction errors, while obtaining reliable geometric priors is costly (mesh modeling demands substantial manual effort).
4. **Paper Goals**: How can readily accessible geometric priors (3D point clouds) be exploited to significantly improve 3D Gaussian generation quality?
5. **Starting Point**: With the proliferation of LiDAR sensors and depth cameras, acquiring clean point cloud data has become highly convenient. Point clouds can serve as reliable geometric priors, reducing the generation task from "joint geometry–appearance learning" to "growing appearance on given geometry."
