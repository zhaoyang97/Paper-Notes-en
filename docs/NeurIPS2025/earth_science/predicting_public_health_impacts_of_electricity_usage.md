...
**Root Cause**: A large fraction of electricity demand is dynamically controllable (e.g., data center loads, EV charging), yet AI tools that directly link electricity consumption decisions to specific health outcomes are absent.

**Starting Point**: Design an end-to-end pipeline that begins from fuel mix time series, proceeds through emission estimation and atmospheric dispersion modeling, and ultimately outputs a health cost signal in $/MWh via health economics valuation.
