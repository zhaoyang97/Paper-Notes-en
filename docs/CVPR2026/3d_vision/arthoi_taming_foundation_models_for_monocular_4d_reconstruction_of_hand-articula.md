...
**State of the Field**: Hand-object interaction (HOI) reconstruction is critical for human behavior analysis, robotic manipulation, and augmented reality. Early methods rely on predefined object templates or category-specific knowledge, limiting generalizability.
...
**Starting Point**: Drawing inspiration from how humans leverage accumulated knowledge and experience to perceive complex interactions, the paper proposes **exploiting rich priors from multiple foundation models** (image-to-3D, pose estimation, depth estimation, tracking, MLLM, etc.) to address this ill-posed problem.

**Root Cause**: Naively integrating multiple foundation models fails because: (1) meshes generated by image-to-3D models are in normalized coordinate spaces and lack metric scale; (2) independently reconstructed hands and objects suffer from spatial misalignment.
