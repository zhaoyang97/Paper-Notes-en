...
- **State of the Field**: Weight-only quantization is the mainstream approach for alleviating memory bottlenecks in LLM inference; codebook-based quantization methods (e.g., AQLM, GPTVQ, QuIP#) maintain competitive accuracy at extremely low bit-widths (e.g., 2-bit) and have become a leading research direction.
...
- **Root Cause**: Codebook quantization achieves superior accuracy over uniform quantization, yet its inference efficiency can be worse—the AQLM 1×16 kernel latency even exceeds the FP16 baseline, completely negating the memory savings from compression.
- **Paper Goals**: Design an efficient codebook-centric GEMM kernel that simultaneously reduces computational complexity and on-chip memory requirements, enabling codebook quantization to achieve both high accuracy and high throughput at extremely low bit-widths.
- **Starting Point**: The number of centroids in a codebook is bounded by $2^b$, while the number of weight matrix rows $M$ greatly exceeds $2^b$; consequently, many codes point to the same centroid, causing their inner products with the input to be recomputed repeatedly. Precomputing and caching these inner products eliminates this redundancy.
