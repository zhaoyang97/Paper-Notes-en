...
**Code**: None
**Area**: Numerical Linear Algebra / Matrix Theory
**Keywords**: Low-rank approximation, pseudoinverse, perturbation bounds, spectral norm, eigengap
...
$$\|(\tilde{A}^{-1})_p - A_p^{-1}\| \leq 5\left(\frac{\|E\|}{\lambda_n^2} + \frac{\|E\|}{\lambda_{n-p} \delta_{n-p}}\right)$$
   where $\delta_{n-p} = \lambda_{n-p} - \lambda_{n-p+1}$ denotes the eigengap.
