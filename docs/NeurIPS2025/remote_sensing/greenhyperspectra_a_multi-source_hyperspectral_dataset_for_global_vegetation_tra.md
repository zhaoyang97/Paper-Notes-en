...
**Root Cause**: Hyperspectral data itself is abundant (unlabeled data is easy to acquire), but annotations are scarce. Existing self/semi-supervised learning in hyperspectral remote sensing primarily targets classification tasks (e.g., land cover mapping), whereas trait prediction is a multi-output regression problem that must additionally handle input heterogeneity across sensors. No large-scale pretraining dataset or self/semi-supervised framework has been designed specifically for plant trait prediction.

**Paper Goals**: (1) Construct a large-scale hyperspectral pretraining dataset spanning multiple sensors and ecosystems; (2) Establish a benchmark framework of semi/self-supervised methods for multi-output regression tasks; (3) Validate the advantages of pretraining under label-scarce and out-of-distribution settings.

**Starting Point**: Leverage 140,000+ unlabeled hyperspectral samples from diverse platforms (proximal/airborne/satellite), time periods (1992–2024), and ecosystems for pretraining, in order to learn spectrally robust, cross-domain representations.
