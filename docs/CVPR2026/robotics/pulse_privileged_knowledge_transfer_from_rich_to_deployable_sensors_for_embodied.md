...
1. **State of the Field**: Multi-sensor systems are widely used in wearable health monitoring, robotics, and related domains. Electrodermal activity (EDA) is the gold-standard physiological indicator of acute stress, as it directly reflects sympathetic nervous system activation.
...
3. **Root Cause**: EDA data is available during training (collected in laboratory settings) but unavailable at deployment. Discarding EDA outright wastes a valuable supervisory signal, while symmetric alignment methods fail to exploit the informational asymmetry of EDA.
4. **Paper Goals**: Leverage privileged EDA information during training to enhance the representation learning of cheap sensors, while requiring no EDA whatsoever at inference time.
5. **Starting Point**: The work builds on the LUPI (Learning Using Privileged Information) paradigm, with a key innovation: rather than naively aligning all modalities, the student representation is decomposed into a *shared* subspace (alignable, used for knowledge transfer) and a *private* subspace (preserving modality-specific structure).
