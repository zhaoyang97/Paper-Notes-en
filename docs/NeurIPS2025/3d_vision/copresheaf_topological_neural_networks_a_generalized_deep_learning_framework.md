...
**Root Cause**: Prevailing models universally assume a single global latent space and isotropic information propagation, which is fundamentally misaligned with the heterogeneous, directional, and hierarchical nature of real-world data. A unified framework that natively encodes diverse local behaviors, respects directional coupling, and propagates information across scales is needed.

**Paper Goals**: To construct a unified deep learning framework based on copresheaves, assigning each local region its own independent feature space and learnable information-transfer maps, thereby naturally supporting multi-scale, anisotropic, and heterogeneous message passing.
