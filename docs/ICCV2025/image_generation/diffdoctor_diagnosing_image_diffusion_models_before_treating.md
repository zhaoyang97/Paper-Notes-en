...
**State of the Field**: Image diffusion models (e.g., FLUX.1, SDXL, Kolors) can generate diverse images but still produce artifacts such as shape distortions (e.g., malformed hands/faces), implausible content (e.g., extra limbs), and watermarks. Existing improvement methods primarily leverage image-level scores or human preference feedback to optimize these models.
...
**Root Cause**: Solving a problem requires first localizing it—fine-tuning a model with image-level scores without knowing where artifacts reside is inefficient and may degrade overall image quality.
