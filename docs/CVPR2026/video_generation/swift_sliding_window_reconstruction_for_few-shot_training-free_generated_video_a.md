...
1. **State of the Field**: Video generation technologies (HunyuanVideo, Wan2.1/2.2, EasyAnimate, etc.) are advancing rapidly, all adopting 3D VAE + DiT architectures. Generated videos risk being misused for disinformation or intellectual property infringement.
...
3. **Root Cause**: Image attribution methods focus solely on spatial consistency, ignoring the temporal consistency constraints inherent in video data, making them ineffective against sequence-correlated perturbations.
4. **Paper Goals**: How to achieve reliable generated video attribution without training and with only a small number of samples, by leveraging the temporal characteristics of video?
5. **Starting Point**: State-of-the-art video generation models employ 3D VAEs that perform temporal up/downsampling (typically with compression ratios of 4 or 8), naturally forming a "$K$ pixel frames $\leftrightarrow$ 1 latent frame" temporal mapping. Videos generated by a target model satisfy that model's VAE distribution when chunk-aligned, whereas non-attributed videos do not.
