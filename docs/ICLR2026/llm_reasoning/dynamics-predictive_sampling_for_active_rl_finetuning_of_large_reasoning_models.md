...
**State of the Field**: RL finetuning, exemplified by GRPO, has become a core technique for improving LLM reasoning. Large reasoning models (LRMs) such as DeepSeek-R1 and OpenAI o1 have achieved breakthroughs in mathematical competition, code generation, and logical inference through RL finetuning. However, the effectiveness of RL finetuning depends critically on the quality of training data selection—not all prompts contribute equally to policy optimization.
