...
**State of the Field**: Synthesizing training data with diffusion models has become a mainstream approach to data augmentation; methods such as Real-Fake achieve domain alignment through fine-tuning.
...
**Root Cause**: Without fine-tuning, domain alignment fails; with fine-tuning, only easy samples are generated. A fundamental trade-off exists between domain alignment and difficulty diversity.

**Paper Goals**: To maintain domain alignment while controlling the learning difficulty of generated samples.

**Starting Point**: Learning difficulty is treated as an explicit conditioning signal injected into the diffusion model, with a dedicated difficulty encoder used to decouple domain alignment from difficulty modeling.
