...
**Root Cause**: Achieving high-ratio KV Cache compression requires accurately identifying which KV pairs are most important for inference; however, the mechanism by which existing methods obtain importance signals (attention scores) is both insufficiently accurate and incompatible with efficient inference frameworks.

**Paper Goals**: To design a compression method that does not rely on attention scores, instead deriving importance directly from the distributional characteristics of the KV matrices themselves—enabling efficient identification of inference-critical KV pairs while remaining natively compatible with FlashAttention and similar efficient attention implementations.

**Starting Point**: Drawing inspiration from frequency-domain analysis in signal processing, the authors transform KV matrices into the frequency domain to observe their energy distribution. They find that the frequency-domain energy of KV matrices is highly concentrated in low frequencies—low-frequency components correspond to smooth, redundant principal-component patterns—while KV pairs that deviate from these principal components ("outlier KVs") are more likely to encode features critical for inference, and preferentially removing them leads to significant performance degradation.
