...
**State of the Field**: Data-Free Continual Model Merging (DFCMM) has emerged as a popular direction in model reuse. The setting involves multiple task-specific models independently fine-tuned, which are then **incrementally merged** into a unified backbone without access to any original task data—motivated by both privacy constraints and storage overhead. The entire merging process must operate solely in parameter space.
