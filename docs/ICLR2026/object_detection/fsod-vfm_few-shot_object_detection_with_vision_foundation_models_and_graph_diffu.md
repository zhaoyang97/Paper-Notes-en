...
**State of the Field**: Few-shot object detection (FSOD) aims to detect novel categories from limited annotated samples. Conventional approaches require fine-tuning, while recent training-free methods leverage foundation models for direct detection.
...
**Root Cause**: Post-processing methods such as SoftNMS consider only spatial relationships between bounding boxes, and cannot exploit semantic or mask-overlap information across proposals to determine which candidate is superior.

**Paper Goals**: How can fragmented proposals be effectively suppressed within a training-free framework to yield high-quality detection results?

**Starting Point**: Model inter-proposal relationships as a directed graph and propagate confidence scores via PageRank-style graph diffusion.
