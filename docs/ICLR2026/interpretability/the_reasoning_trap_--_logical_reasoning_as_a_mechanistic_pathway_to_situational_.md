...
**Root Cause**: Logical reasoning exhibits **domain generality**—the validity of modus ponens is independent of its subject matter. A model that has mastered deductive reasoning for medical diagnosis simultaneously possesses the same rules for inferring properties of its own training. This non-separability implies that advances in reasoning capability necessarily enhance self-directed reasoning as well.

**Starting Point**: The authors invoke the Sherlock Holmes analogy—"sufficiently powerful reasoning, combined with minimal evidence, can generate knowledge far exceeding what direct observation would allow." A reasoning model needs only sparse cues about itself extracted from interactions to construct a comprehensive picture of its own nature through inference.
