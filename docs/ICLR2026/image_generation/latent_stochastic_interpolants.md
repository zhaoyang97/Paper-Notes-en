...
- **Key Challenge**: Running SI directly in a high-dimensional observation space is computationally expensive. Attempting to use a latent space to reduce cost often fails because the posterior is "dynamic and unobservable." Existing latent diffusion models often revert to simple Gaussian priors or rely on ad-hoc multi-stage training (pre-training an autoencoder before the generator), which might result in a misalignment between the latent representation and the generative process.
