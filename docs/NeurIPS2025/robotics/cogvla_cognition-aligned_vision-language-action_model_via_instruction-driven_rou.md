...
**State of the Field**: VLA models (e.g., OpenVLA, π₀, RT-2) unify vision, language, and action under pretrained VLMs for end-to-end robotic control. However, post-training adaptation to action spaces is computationally prohibitive—fine-tuning a 7B VLA model on a single LIBERO task, for instance, requires over 600 A100 GPU hours.
