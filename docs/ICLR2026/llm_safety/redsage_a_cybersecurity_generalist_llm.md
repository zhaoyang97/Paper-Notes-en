...
**State of the Field**: Cybersecurity threats are growing in complexity, and tasks such as APT attack analysis, vulnerability management, and incident response demand deep domain expertise and tool-operating capabilities. The global cybersecurity talent gap has reached millions (ISC² report), driving demand for LLM-assisted security analyst tools. Several cybersecurity LLMs have emerged in recent years (Foundation-Sec, PRIMUS, DeepHat, etc.), yet each suffers from notable shortcomings.
