...
**Paper Goals**: (1) Formalize the "shared Gaussian latent space" hypothesis and provide empirical support; (2) verify whether DDIM inversion noise from diffusion models encodes semantic information comparable to that of encoders; (3) demonstrate the feasibility of direct linear semantic editing in the noise space.

**Starting Point**: Generative models sample from Gaussian noise to produce images, while encoders map images to embeddings that are empirically approximately Gaussian — these two directions are in fact two "perspectives" on the same Gaussian latent space. The authors formalize this as the Induced Normal Embedding (INE): each model's latent space is a noisy linear projection of the ideal UNE.
