...
**State of the Field**: COLMAP is the de facto standard for camera parameter estimation in static scenes, but handling dynamic scenes requires ground-truth motion masks to exclude moving objects. Numerous improved methods have emerged (categorized in Table 1), yet the vast majority rely on additional priors: GT focal length (CF-3DGS, Nope-NeRF), GT motion masks (GFlow, LEAP-VO), metric depth (DROID-SLAM), or GT 3D point clouds and poses (DUSt3R, Monst3R, Cut3R). Such priors are generally unavailable in casually captured videos.
