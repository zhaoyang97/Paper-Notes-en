...
1. **State of the Field**: Sound source localization (SSL) aims to identify the spatial origin of sounds in images by associating audio and visual information. Existing methods fall into two main categories: contrastive learning-based single-source methods and pseudo-label/graph-based multi-source methods, both fundamentally relying on feature matching.
...
3. **Root Cause**: Human sound source localization involves a multi-step reasoning process—perceiving audio-visual signal characteristics, systematically analyzing candidate objects, and refining conclusions. This interpretable verification process far exceeds simple matching, yet existing methods entirely overlook it.
4. **Paper Goals**: How can MLLM reasoning capabilities be exploited for interpretable SSL without any training? Specifically: (a) how to generate candidate sound sources; (b) how to verify the plausibility of candidates; and (c) how to refine localization results.
5. **Starting Point**: Inspired by human metacognitive processes, the authors observe that MLLMs already possess strong cross-modal understanding, structured reasoning, and instruction-following capabilities, enabling them to serve directly as reasoning engines rather than auxiliary encoders.
