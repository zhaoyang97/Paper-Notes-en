...
**State of the Field**: Robustness research on visual encoders—particularly CLIP and its variants—has predominantly focused on model sensitivity to image transformations and corruptions (e.g., blur, noise, contrast shifts). Such studies typically target severe, perceptually salient distortions unseen during training, finding that they introduce distribution shift and degrade performance.
