...
3. **Root Cause**: How can multi-reference video content customization be achieved without architectural modification and without relying on large-scale custom datasets?
4. **Paper Goals**: (a) Understand the intrinsic capabilities of video generation models—can the first frame serve as a concept store? (b) How can this capability be reliably activated? (c) How can multi-scenario generalization be achieved while preserving pretrained knowledge?
5. **Starting Point**: The authors identify a previously overlooked phenomenon: pretrained image-to-video (I2V) models already possess the latent ability to extract visual concepts from a collage first frame and fuse them coherently in subsequent frames. However, this ability is difficult to trigger reliably via prompt engineering alone (instability, loss of object identity). A lightweight LoRA adaptation on a minimal number of training samples suffices to activate it reliably.
