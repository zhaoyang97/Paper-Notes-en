...
- **Root Cause**: Raw XLFM acquisition is cheap and abundant, but annotations are expensive and standardized benchmarks are absent — data-rich yet label-scarce. Models trained with purely pixel-level losses may generate visually plausible but optically inconsistent "hallucinated" structures, undermining scientific credibility.
- **Paper Goals**: How can one efficiently learn angular priors of XLFM under label-scarce conditions while ensuring physical consistency of the reconstructed volume?
- **Starting Point**: XLFM reconstruction is reframed as a **structured prediction problem** — using "views" rather than "pixels" as the atomic modeling unit for self-supervised pretraining, and introducing differentiable forward rendering constraints based on the known PSF.
