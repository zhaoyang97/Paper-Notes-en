...
**Root Cause**: Detectors may perform well on aggregate metrics, yet such aggregated scores conceal substantial disparities across subgroups. Deploying detectors without fairness auditing systematically penalizes certain groups in high-stakes contexts such as educational grading and content moderation.

**Paper Goals**: To construct a standardized benchmark covering multiple bias dimensions and systematically quantify performance disparities across sociolinguistic subgroups.

**Starting Point**: Expanding the bias dimension from ELL alone to 7 major categories (demographics, age, grade level, dialect, formality, political leaning, and topic), and generating semantically aligned AI counterparts for each human-authored text.
